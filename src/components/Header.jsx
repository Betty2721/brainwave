...
const Header = () => {
    ...
    const handleClick = () => {
      if (!openNavigation) return;
        ...
        enablePageScroll();
        setopenNavigation(false); // close the menu automatically when  a menu clicked
    };

  return (
    <div className={`fixed top-0 left-0 w-full z-50
      border-b border-n-6 lg:bg-n-8/90
      lg:backdrop-blur-sm ${openNavigation ? 'bg-n-8' : 
      'ng-n-8/90 backdrop-blur-sm'}`}>
        ...
        <a
          ...
          className="button hidden mr-8 text-n-1/50
             transtion-colors hover:text-n-1 lg:block"
             >
            New account
        </a>
